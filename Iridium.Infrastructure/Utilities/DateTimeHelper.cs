namespace Iridium.Infrastructure.Utilities
{
    public class DateTimeHelper
    {
        ...
        public static bool OverlappingPeriods(DateTime Period1Start, DateTime Period1End, DateTime Period2Start, DateTime Period2End)
        {
    ...
    {
        private static string FormatPart(int quantity, string name) => quantity > 0 ? $"{quantity} {name}" : null;

        public static string ToPrettyFormat(this TimeSpan timeSpan)
        {
            return string.Join(", ", new[] { FormatPart(timeSpan.Days, "gün"), FormatPart(timeSpan.Hours, "saat"), FormatPart(timeSpan.Minutes, "dakika") }.Where(x => x != null));
        }
    }
    ...
    public class Period
    {
        ...
        public bool IsOverlapping(Period period) => DateTimeOverlapHelper.IsOverlapping(this, period);
    }

    public class DateTimeOverlapHelper
    {
        public static bool IsOverlapping(Period first, Period second) =>
            Math.Max(first.StartDate.Ticks, second.StartDate.Ticks) <= Math.Min(first.EndDate.Ticks, second.EndDate.Ticks);
    }
}
